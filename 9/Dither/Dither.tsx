...
console.log("Dither component file loaded");
...
const DitheredWaves: React.FC<DitheredWavesProps> = ({
  waveSpeed = 0.05,
  waveFrequency = 3,
  waveAmplitude = 0.3,
  ...
  console.log("DitheredWaves component initializing with props:", {
    waveSpeed,
    waveFrequency,
    waveAmplitude,
    waveColor,
  ...
  try {
    ...
    const waveUniforms = useRef({
      ...
      u_time: { value: 0 },
      u_wave_speed: { value: waveSpeed },
      u_wave_frequency: { value: waveFrequency },
      u_wave_amplitude: { value: waveAmplitude },
      u_wave_color: { value: new THREE.Vector3(...waveColor) },
      u_mouse: { value: new THREE.Vector2(0, 0) },
      u_mouse_radius: { value: mouseRadius },
      u_enable_mouse: { value: enableMouseInteraction ? 1.0 : 0.0 },
    });

    useEffect(() => {
      try {
        console.log("Setting up resolution in DitheredWaves useEffect");
        const dpr = window.devicePixelRatio;
        waveUniforms.current.u_resolution.value = [size.width * dpr, size.height * dpr];
        console.log("Resolution set to:", waveUniforms.current.u_resolution.value);
      } catch (error) {
        console.error("Error in resolution useEffect:", error);
      }
    }, [size]);
    ...
    useFrame((state, delta) => {
      try {
        ...
        if (waveUniforms.current) {
          waveUniforms.current.u_time.value += delta;
        }

        // Update effect uniforms if needed
        if (effect.current) {
    ...
    const handlePointerMove = useCallback(
      (event: ThreeEvent<PointerEvent>) => {
        try {
          ...
          if (waveUniforms.current) {
            waveUniforms.current.u_mouse.value.set(x, y);
          }
        } catch (error) {
          console.error("Error in handlePointerMove:", error);
        }
      },
      [viewport, enableMouseInteraction]
    );

    console.log("DitheredWaves about to render");
    ...
    console.error("Fatal error in DitheredWaves component:", error);
    return <mesh>
...
export default function Dither({
	...
	console.log("Dither component rendering", { waveSpeed, waveFrequency, waveAmplitude });
		...
		return (
			<div className="w-full h-full bg-gradient-to-br from-purple-900 to-blue-900 flex items-center justify-center">
				<div className="text-white text-center">
					<p>Could not load 3D background</p>
				</div>
			</div>
		);
	}
